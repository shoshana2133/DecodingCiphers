...
namespace frequency
{
    static public class Replacement
    {
        ...
        //מערך התורים שבהם המילים עם אות אחת לא מפוענחת
        static Queue<int>[] Queue = new Queue<int>[15];
        ...
        {
            Dic_exchange['ו'] = Translate.Dic_exper_exchange.ElementAt(0).Key;
            LocationAndUpdate('ו', Dic_exchange['ו']);
            Dic_exchange['י'] = Translate.Dic_exper_exchange.ElementAt(1).Key;
            LocationAndUpdate('י', Dic_exchange['י']);
            Dic_exchange['ה'] = Translate.Dic_exper_exchange.ElementAt(2).Key;
            LocationAndUpdate('ה', Dic_exchange['ה']);
            Dic_exchange['מ'] = Translate.Dic_exper_exchange.ElementAt(4).Key;
            LocationAndUpdate('מ', Dic_exchange['מ']);
            Dic_exchange['ל'] = Translate.Dic_exper_exchange.ElementAt(5).Key;
...

        {
            for (int i = 0; i < Words.Count(); i++)
            {
                ...
                // שליחה לפונקציה שממקמת את האות במילה ומחזירה את מספר האותיות שנותרו לפיענוח
                {
        ...
        public static int Location(char tav, char tav2, int j)
        {
            ...
            return Words[j].LettersRemain==1?1:caunt;
        }
        ...
        public static bool Try(int ind)
        {
            ...
            for (char t = 'א'; t <= 'ת'; t++)
            {
                ...
                help = copyWord.Replace('?', t);
                if (CutWords.isExists(help) == true)
                {
                    count++;
                    index = t;
                }
            ...
            if (count == 1)
            {
                //איפוס תוכן התורים הקודמים
                jj();
                Dic_exchange[index] = Words[ind].CipherWord[Words[ind].Decoding.IndexOf("?")];
                cou++;
                //שליחה לפונקציית עדכון המילים
                LocationAndUpdate(index, Words[ind].CipherWord[Words[ind].Decoding.IndexOf("?")]);
                return true;
            }
            return false;
        }
